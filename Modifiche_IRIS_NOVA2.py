...
# - nuovo stato iniziale per il settaggio dei motori e il controllo delle connessioni del PV (completato)
...
class IRIS_FSM(fsmBase):
    def __init__(self, name, *args, **kwargs):
        ...
        self.exact_2_charge_central_steps = 16640 # 16320 
        self.exact_3_charge_central_steps = 24960 # 24768
...
# Scarica 3^ e ultimo Target: Charge Buffer, m2 - - - - - - - - - - - - - - - - - - - - - - 
    ...
    def  Charge_Buffer3_state_exit(self):
           pass
# nel precedente stato per far entrare la
# compressa è necessario toccare il limit switch
# il limit switch si attiva sul reverse e non sul forward
# quindi il motore non può più fare movimenti reverse ma solo forward
# quindi saltiamo la correzione al momento e andiamo direttamente al solenoide
# dopo il solenoide facciamo un nuovo stato in cui si va avanti tutta
# così sblocchiamo il limit switch e poi lascio continuare il codice come prima
# penso che così facendo rendiamo anche l'esecuzione più veloce
# dato che ha meno strada da fare dopo per fare l'homing reverse
# Correzione Central Movement
...
# Estrazzione attuatore "lock1"
    ...
    def Extract_lock1_1_eval(self):
        if self.lock1_extract.putCompleting():  
            self.logI("1^ - attuator extracted")        
